...
public:
	static std::vector<Vector2D> getRandomPoints(uint amount, uint xBound, uint yBound, ulong seed);

	static Capturing2D generateRandomSharedEndPointsInstance(const std::vector<Vector2D> &points, ulong segmentAmount,
															 ulong seed, uint k = 5);
	static Capturing2D generateRandomSegmentInstance(uint amount, uint xBound, uint yBound, ulong seed, uint k);
	static Capturing2D
	generateProbabilisticInstance(const std::vector<Vector2D> &points, double probability, ulong seed, uint k = 5);
	static Capturing2D generateRandomAllNodesInstance(const std::vector<Vector2D> points, ulong segmentAmount,
													  ulong seed, uint k = 5);
private:
