...
CapturingSolution Metaheuristic::EvolutionComputation::solve(Capturing2D *problem) {
	auto begin_time = std::chrono::high_resolution_clock::now();

	Solutions population;
	std::cout << "Create population. Amount: " << settings.startPopulation << std::endl;
	generatePopulation(&population,*problem);
...
void innerGeneratePopulation(Solutions *population, Capturing2D &problem,ulong seed, ulong iterations,ulong maxSegments) {
	...
	for (uint j = 0; j < iterations; ++j) {
		...
		population->push_back(problem.solveGreedy(segIndex));
	}
}

void Metaheuristic::EvolutionComputation::generatePopulation(std::vector<CapturingSolution> *population, Capturing2D &problem) {
	...
	if(threadsSupported > 1) {
		...
		}
		{
			...
			for (uint j = 0; j < threadsSupported; ++j) {
				threadSolutions.push_back(
						std::async(std::launch::async,&innerGeneratePopulation, &(solutions[j]),std::ref(problem), settings.seed + j, counter[j],maxSegments));
			}
		}
		for (uint j = 0; j < solutions.size(); ++j) {
			for (uint k = 0; k < solutions[j].size(); ++k) {
		...
		innerGeneratePopulation(population,problem, settings.seed, iterations,maxSegments);
	}
...
void innerMutateLocalRandom(Solutions *population, ulong begin, ulong end, ulong seed,Metaheuristic::SASettings saSetting, Capturing2D *problem) {
	for (ulong j = begin; j < end; ++j) {
		Metaheuristic::SimulatedAnnealing simulatedAnnealing;
		simulatedAnnealing.setSettings(saSetting);

		(*population)[j] = simulatedAnnealing.solve(problem,&(*population)[j]);
	}
}
...
void Metaheuristic::EvolutionComputation::mutateLocalRandom(Solutions *population, Capturing2D *problem) {
	...
	if(threadsSupported > 1) {
		...
		}
		{
			...
			for (uint j = 0; j < threadsSupported; ++j) {
				threadSolutions.push_back(
						std::async(std::launch::async,&innerMutateLocalRandom, population, allCount, allCount + counter[j],settings.seed+j,settings.innerSASetting,problem));
				allCount += counter[j];
			}
		}
	} else {
		innerMutateLocalRandom(population,0,population->size(),settings.seed,settings.innerSASetting,problem);
	}
}
