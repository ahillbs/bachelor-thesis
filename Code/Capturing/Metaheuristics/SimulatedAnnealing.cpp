...
void Metaheuristic::computeSolution(CapturingSolution *sol, ulong innerSeed, SASettings settings) {
	...
	while (i < settings.maxIteration && noIncrements < settings.maxNoIncrements && seconds < settings.maxTime) {
		...
		CapturingSolution innerSol = currentSol.getRandomLocalNeighbour(i, settings.useLocal);
...
CapturingSolution Metaheuristic::SimulatedAnnealing::solve(Capturing2D *problem) {
	return solve(problem, nullptr);
}

CapturingSolution Metaheuristic::SimulatedAnnealing::solve(Capturing2D *problem,
														   const CapturingSolution *paramSol) {
