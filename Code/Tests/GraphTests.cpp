...
#include "../Capturing/Generator/Header/Cap2DGenerator.h"
#include "../Capturing/Metaheuristics/SimulatedAnnealing.h"
#include "../Capturing/Metaheuristics/TabuSearch.h"
#include "../Capturing/Wrapper/SolverWrapper.hpp"
#include "../Capturing/Metaheuristics/EvolutionComputation.h"
#include "../MacroTestingFramework/MacroTestingInstance.hpp"
#include "../MacroTestingFramework/TestInstanceContainer.h"
#include "../Capturing/Wrapper/GeneratorWrapper.hpp"
...
namespace {
	...
	TEST(ApproxVsCplexTest,Test10RandomInstances) {
		...
		for (int i = 0; i < 10; ++i) {
			Capturing2D capGraph = Capturing2D(10);
			std::fstream stream;
	...
	TEST(GraphTests,svgImportTest) {
		...
		Capturing2D graph = Imports::importSVG(myfile);
		graph.convertSegments2Graph();
	...
	TEST(GraphTests,csvImportTest) {
		...
		Capturing2D graph = Cap2DGenerator::generateRandomAllNodesInstance(points,200,30);
		Capturing2D graph2 = Cap2DGenerator::generateRandomSharedEndPointsInstance(points,200,30);
		graph.convertSegments2Graph();
	...
	TEST(GraphTests,solutionNeighbourTest) {
		...
		ulong seed = 5;
		ulong k = 10;
		Capturing2D capGraph = Cap2DGenerator::generateProbabilisticInstance(Cap2DGenerator::getRandomPoints(50,1000,1000,seed),20,seed+1,k);
		capGraph.convertSegments2Graph();
		auto greedySol = capGraph.solveGreedy();
		...
		auto simAnnealSol = simulatedAnnealing.solve(&capGraph, nullptr);
	...
	TEST(GraphTests,solutionLocalGlobalNeighbourIteratedTest) {
		...
		for (int j = 0; j < 10; ++j) {
			...
			Capturing2D capGraph = Cap2DGenerator::generateProbabilisticInstance(
					Cap2DGenerator::getRandomPoints(50, 1000, 1000, seed), 30, seed + 1 + j, k+j);
			capGraph.convertSegments2Graph();
			Capturing2D_Random_solution randSolGen;
			auto randSol = randSolGen.solve(&capGraph);
	...
	TEST(GraphTests,solutionLocalGlobalNeighbourSimulatedTest) {
		...
		for (int j = 0; j < 10; ++j) {
			...
			auto begin_time1 = std::chrono::high_resolution_clock::now();
			auto simulSol = simulatedAnnealing.solve(&capGraph, &greedySol);
			auto end_time1 = std::chrono::high_resolution_clock::now();
			...
			auto begin_time2 = std::chrono::high_resolution_clock::now();
			auto simulLocalSol = simulatedAnnealing.solve(&capGraph, &greedySol);
			auto end_time2 = std::chrono::high_resolution_clock::now();
	...
	TEST(GraphTests,solutionRandomStartGreedyTest) {
		...
		for (uint j = 0; j < 10; ++j) {
			...
			Capturing2D capGraph = Cap2DGenerator::generateProbabilisticInstance(
					Cap2DGenerator::getRandomPoints(50, 1000, 1000, seed + j), 30, seed + 1 + j, k+j);
			capGraph.convertSegments2Graph();
	...
	TEST(GraphTest,EvolutioComputation) {
		...
		for (uint j = 0; j < 10; ++j) {
			...
			CapturingSolution evoSol = evolutionComputation.solve(&capGraph);
	...
	TEST(GraphTest,FirstGraphTest) {
		...
		Capturing2D capturing2D(3,segments,8*20,8*20);
	...
	TEST(GraphTests,GeneratorDrawTest) {
		...
		for (uint j = 5; j <= segmentAmount; j += 5) {
			...
			auto rAInstance = randomAllNodesInstanceGenerator.generateInstance(instances[0]->points,instances[0]->amount,j,seed);
