...
TEST(TestFrameworkTests,solverTest) {
	...
	if(problemInstances.size() > 0) {
		...
		CapturingSolution GreedySol = solverGreedy.solve(&problem);
		std::cout << "Solution Greedy: " << GreedySol.getSolutionValue() << std::endl;
		CapturingSolution IteratedSol = solverIterated.solve(&problem);
		...
		CapturingSolution IPSol = solverIP.solve(&problem);
...
TEST(TestFrameworkTests,EvolutionTest) {
	...
	std::vector<TestInstance<NT>*> instances = {&randPointsInstance};
